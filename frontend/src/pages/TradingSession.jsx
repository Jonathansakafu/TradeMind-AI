...
// Pocket Option / Expert Option Quick Trade pairs. OTC pairs are synthetic
// (broker-generated) prices available 24/7, including weekends, which is
// why they're listed separately from the regular market-hours pairs below.
// Limited to pairs our market data provider actually covers — the AI signal
// for each one is grounded in that pair's real underlying price, so only
// listing pairs we can genuinely fetch data for.
const QUICK_TRADE_OTC_PAIRS = [
  "EUR/USD OTC", "GBP/USD OTC", "USD/JPY OTC", "AUD/USD OTC", "USD/CAD OTC",
  "USD/CHF OTC", "NZD/USD OTC", "EUR/JPY OTC", "GBP/JPY OTC", "Gold OTC",
];
const QUICK_TRADE_MARKET_PAIRS = [
  "EUR/USD", "GBP/USD", "USD/JPY", "AUD/USD", "USD/CAD", "USD/CHF", "NZD/USD", "EUR/JPY",
];
      ...
      {!session && (
        <div className="max-w-2xl space-y-6">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6">
            ...
            {mode === "quick_trade" ? (
              <>
                <p className="text-xs font-semibold text-slate-400 dark:text-slate-500 mb-2">OTC (24/7, including weekends)</p>
                <div className="flex flex-wrap gap-2 mb-4">
                  {QUICK_TRADE_OTC_PAIRS.map((pair) => (
                    <button
                      key={pair}
                ...
                <p className="text-xs font-semibold text-slate-400 dark:text-slate-500 mb-2">Market hours</p>
                <div className="flex flex-wrap gap-2">
                  {QUICK_TRADE_MARKET_PAIRS.map((pair) => (
                    <button
                      key={pair}
                      onClick={() => togglePair(pair)}
                      className={`px-3 py-2 rounded-lg text-xs font-semibold transition ${
