...
const TradeSnapshot = forwardRef(({
  ...
  return (
    <div ref={ref} className="bg-slate-950 border border-slate-800 rounded-2xl p-6 w-full">
      {/* Header */}
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-2">
          <span className="text-xl font-bold text-white">{pair || "—"}</span>
          <span className={`flex items-center gap-1 text-sm font-bold px-2.5 py-1 rounded-lg ${
            direction === "buy" ? "bg-green-500/10 text-green-400" : "bg-red-500/10 text-red-400"
          }`}>
      ...
      {hasRange && (
        <div className="mb-6">
          <div className="relative h-3 bg-slate-800 rounded-full overflow-visible mt-8 mb-8">
            <div className="absolute inset-0 rounded-full bg-gradient-to-r from-red-500/30 via-slate-700/30 to-green-500/30" />
            <div className="absolute top-0 bottom-0 w-0.5 bg-red-500" style={{ left: `${posOf(sl)}%` }}>
              <span className="absolute -top-6 -translate-x-1/2 text-[11px] font-semibold text-red-400 whitespace-nowrap">SL {sl}</span>
            </div>
            <div className="absolute top-0 bottom-0 w-0.5 bg-blue-400" style={{ left: `${posOf(entryNum)}%` }}>
              <span className="absolute -top-6 -translate-x-1/2 text-[11px] font-semibold text-blue-400 whitespace-nowrap">Entry {entryNum}</span>
            </div>
            <div className="absolute top-0 bottom-0 w-0.5 bg-green-500" style={{ left: `${posOf(tp)}%` }}>
              <span className="absolute -top-6 -translate-x-1/2 text-[11px] font-semibold text-green-400 whitespace-nowrap">TP {tp}</span>
            </div>
            {marker != null && (
              <div
                className={`absolute top-1/2 -translate-y-1/2 w-4 h-4 rounded-full border-2 border-slate-950 ${
                  outcome === "win" ? "bg-green-400" : outcome === "loss" ? "bg-red-400" : "bg-white"
                }`}
                style={{ left: `${posOf(marker)}%`, transform: "translate(-50%, -50%)" }}
              >
                <span className="absolute -bottom-6 -translate-x-1/2 left-1/2 text-[11px] font-bold text-white whitespace-nowrap">
                  {markerLabel} {marker}
                </span>
              </div>
            )}
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-3 text-center">
          ...
          <p className="font-mono font-bold text-blue-400 text-sm">{entry || "—"}</p>
        </div>
        <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-3 text-center">
          <p className="text-xs text-slate-500 mb-1">Stop Loss</p>
          <p className="font-mono font-bold text-red-400 text-sm">{stopLoss || "—"}</p>
        </div>
        <div className="bg-green-500/10 border border-green-500/20 rounded-xl p-3 text-center">
          <p className="text-xs text-slate-500 mb-1">Take Profit</p>
          <p className="font-mono font-bold text-green-400 text-sm">{takeProfit || "—"}</p>
        </div>
      </div>
        ...
        <div className={`rounded-xl p-4 border ${
          ...
          <div className="grid grid-cols-3 gap-3 text-center">
            <div>
              ...
              <p className="font-mono font-bold text-white text-sm">{markerPrice || "—"}</p>
            </div>
            <div>
              <p className="text-xs text-slate-400 mb-1">P&amp;L</p>
              <p className={`font-mono font-bold text-sm ${Number(pnl) >= 0 ? "text-green-400" : "text-red-400"}`}>
                {Number(pnl) >= 0 ? "+" : ""}{pnl != null ? `$${pnl}` : "—"}
              </p>
            </div>
            <div>
              <p className="text-xs text-slate-400 mb-1">Pips</p>
              <p className={`font-mono font-bold text-sm ${Number(pips) >= 0 ? "text-green-400" : "text-red-400"}`}>
                {Number(pips) >= 0 ? "+" : ""}{pips ?? "—"}
              </p>
            </div>
          </div>
